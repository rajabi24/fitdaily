...
<div class="mb-10 fade-in">
    ...
    <h1 class="text-4xl font-extrabold text-slate-800 mb-2">Edit Jadwal Latihan</h1>
    <p class="text-slate-500">Ubah jadwal latihan yang sudah ada.</p>
</div>
...
<div class="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm fade-in-delay-1">
    ...
    <form action="/workouts/{{ $workout->id }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="day" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:border-blue-400">
                    @foreach(['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'] as $day)
                        <option value="{{ $day }}" {{ $workout->day == $day ? 'selected' : '' }}>{{ $day }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="category" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:border-blue-400">
                    @foreach(['Gym','Running','Rest'] as $cat)
                        <option value="{{ $cat }}" {{ $workout->category == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="exercise_name" value="{{ $workout->exercise_name }}" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:border-blue-400">
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Durasi (menit)</label>
                <input type="number" name="duration" value="{{ $workout->duration }}" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:border-blue-400">
            </div>
            ...
            <div>
                ...
                <select name="intensity" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:border-blue-400">
                    @foreach(['Ringan','Sedang','Berat','-'] as $intensity)
                        <option value="{{ $intensity }}" {{ $workout->intensity == $intensity ? 'selected' : '' }}>{{ $intensity }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="3" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:border-blue-400">{{ $workout->description }}</textarea>
            </div>
        </div>
